...
- Gaussian-Only 모드 기반 실시간 3D 상대 치수 측정
...
- ❌ PLY 전처리 / 절대 부피 DB 매칭 / OBB→m³ 변환 → 서비스 레이어 (논문 스코프 밖)
...
#### Abstract (150-200자)
본 논문은 이삿짐 견적 등 3D 상대 치수 추출이 필요한 서비스를 위해 SAM-3D의 도메인 특화 경량화 방법을 제안한다.
범용 3D 생성 모델인 SAM-3D는 고품질 mesh/texture 복원을 목표로 설계되어 객체당 150초, VRAM 21GB를
요구하므로 실서비스 적용이 어렵다. 우리는 **상대 치수 추출에는 3D point cloud만 필요하다는 관찰**에
기반해 mesh 디코딩 경로를 완전히 우회하는 **Gaussian-Only 모드**, 이로 인해 dead-code가 된
서브 모델을 언로드하는 **VRAM 최적화**, diffusion step의 **상대 치수 기준 최적점 탐색**을 제안한다.
실험 결과 L4 GPU에서 객체당 처리 시간을 150초에서 13초로 단축(11.5배 가속)하고
...
- **기여 3가지**:
  1. Gaussian-Only 디코딩으로 SAM-3D의 mesh 경로 완전 우회 (37% 가속)
  2. dead-code path가 된 SLaT mesh/GS-4/depth 모델 언로드로 48% VRAM 절감
  3. 상대 치수 기준 diffusion steps 최적점 탐색 (시각 품질 기준보다 공격적 축소 가능)
...
### 핵심 관점: Original SAM-3D 출력을 baseline으로 상대 치수 비교

본 논문의 스코프는 **SAM-3D 모델 최적화**입니다.
- 다루는 것: SAM-3D → Gaussian Splat → OBB → **상대 치수 (w, d, h)** 까지
- 다루지 않는 것: 표준 치수 DB 매칭, 절대 부피(m³) 역산 — 서비스 레이어

따라서 claim은 "실제 가구와 얼마나 같은가"가 **아니라**
"**최적화해도 원본 SAM-3D의 상대 치수 출력이 유지되는가**"입니다.

3D 복원 품질(CD, F-Score 등)은 **SAM-3D 원 논문이 이미 검증**한 것이고,
우리는 그걸 전제로 **"최적화해도 OBB 상대 치수가 보존됨"만 실증**하면 됩니다.
...
| 항목 | 실측 GT 기반 비교 | ✅ Original SAM-3D 대비 비교 (채택) |
|------|--------------|--------------------------|
...
2. 우리는 downstream에서 OBB 상대 치수(w, d, h)만 보존하면 됨
3. 우리의 최적화가 상대 치수를 얼마나 변화시키나? → Table 2의 W/D/H err
4. 변화가 <3%이면, 원본 SAM-3D의 3D 품질이 상대 치수 수준에서 보존됨
5. 결론: 11.5× 가속 @ <3% 상대 치수 편차 → training-free task-aware pruning 유효
```

이 논리는 심사자가 "실측 GT와 비교 안 하나?" 라는 질문에 대해
**"SAM-3D 원 논문이 3D 복원 품질을 보장했고, 본 연구는 그 출력의 OBB 상대 치수가 최적화 후에도 유지됨을 보입니다. 절대 부피(m³) 산출은 서비스 레이어의 DB 매칭 단계이며 본 논문의 스코프 밖입니다."**
라고 깔끔하게 방어 가능합니다.
