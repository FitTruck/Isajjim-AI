...
**전체 복잡도: O(n² × m × r)**
- n = 아이템 수
- m = Extreme Points 수 (최대 3n)
- r = 회전 방향 수 (수평만: 2, 전체: 6)

| 단계 | 복잡도 | 설명 |
|------|--------|------|
| 정렬 | O(n log n) | 부피 기준 내림차순 정렬 |
| 아이템 순회 | O(n) | 각 아이템에 대해 |
| ├─ EP 순회 | O(m) | 모든 EP 후보 검사 |
| │  ├─ 회전 시도 | O(r) | 각 회전 방향 시도 |
| │  ├─ 경계 검사 | O(1) | 트럭 범위 확인 |
| │  ├─ 충돌 검사 | O(n) | 모든 배치된 박스와 비교 |
| │  └─ 지지 검사 | O(n) | 아래 박스들과 겹침 계산 |
| └─ EP 업데이트 | O(m) | 무효 EP 제거 + 정렬 |

**단계별 분석:**

```
1. 정렬: O(n log n)
2. 메인 루프: n × (m × r × n + m) = O(n² × m × r)
...
| 자료구조 | 크기 | 설명 |
|----------|------|------|
| placed_items | O(n) | 배치된 박스 리스트 |
| extreme_points | O(m) ≈ O(3n) | EP 후보 리스트 |
| sorted_items | O(n) | 정렬된 아이템 (참조) |

### 최적화 기법

1. **Early Termination**: 첫 번째 유효한 배치 발견 시 중단 가능 (현재 미적용)
2. **EP 정렬**: 우선순위 기반 정렬로 좋은 위치 먼저 검사
3. **무효 EP 제거**: 박스 내부 EP 즉시 제거로 검색 공간 축소

### 요약

| 항목 | 값 |
|------|-----|
| 시간 복잡도 | O(n³) (실질적으로 O(n² × m)) |
| 공간 복잡도 | O(n) |
| 일반적인 적재율 | 50-70% |
| 처리 시간 (20개) | < 100ms |
